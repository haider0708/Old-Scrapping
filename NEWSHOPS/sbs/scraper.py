"""
SBS Informatique e-commerce web scraper.
...
# -----------------------------------------------------------------------------
# Categories (Playwright, CSR)
# -----------------------------------------------------------------------------


async def scrape_categories(pool: BrowserPool, semaphore: asyncio.Semaphore) -> list[dict]:
    ...
    try:
        async with semaphore:
            ...
            await page.wait_for_selector(
                config.PLAYWRIGHT_CATEGORY_WAIT,
    ...
    cat_li_items = nav.css("li[data-depth]")
    for li in cat_li_items:
        ...
        if not a_el:
            continue
        cat_url = _abs_url(parse_node_attr(a_el, "href"))
        cat_name = parse_node_text(a_el)
        cat_id = _extract_id_from_url(cat_url) or _extract_slug_from_url(cat_url) or f"cat_{len(categories)}"
        if not cat_url or cat_url in seen_urls:
            continue
...
def scrape_product_detail(html: str, url: str) -> dict:
    ...
    stock_bar_el = tree.css_first(sel.get("stock_bar", ""))
    stock_level = ""
    if stock_bar_el:
        classes = (stock_bar_el.attributes or {}).get("class") or ""
...
async def main() -> None:
    ...
    try:
        ...
        async with BrowserPool(config.BROWSER_POOL_SIZE) as pool:
            ...
            categories = await scrape_categories(pool, semaphore)
