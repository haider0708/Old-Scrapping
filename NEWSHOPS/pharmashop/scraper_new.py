"""Pharmashop — Shop-specific scraper. PrestaShop 1.7 + Leo Theme. Full SSR. JSON detail."""
...
class PharmashopScraper(BaseScraper):
    shop_name = "pharmashop"
    shop_config = config
    ...
    def scrape_listing_page(self, html: str, category_url: str) -> list[dict]:
        ...
        for elem in tree.css(ls["element"]):
            ...
            price_numeric = price
            meta = elem.css_first(ls.get("price_schema", ""))
            if meta:
            ...
            availability = ""
            if av_cfg.get("out_of_stock_flag") and elem.css_first(av_cfg["out_of_stock_flag"]):
            ...
            products.append({
                ...
                "old_price": old_price, "brand": brand, "availability": availability,
            })
        return products
    ...
    def scrape_product_detail(self, html: str, url: str) -> dict:
        ...
        if not images_list:
            imgs = sel.get("images", {})
            for img in tree.css(imgs.get("thumbnails", "")):
                u = parse_node_attr(img, "src") or parse_node_attr(img, "data-src")
                if u and u not in images_list: images_list.append(abs_url(u, _base))
            if not images_list:
                main_el = tree.css_first(imgs.get("main", ""))
                if main_el:
                    u = parse_node_attr(main_el, "src")
                    if u: images_list.append(abs_url(u, _base))
