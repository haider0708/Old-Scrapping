"""
Pharmacie+ e-commerce web scraper.
Full CSR - ALL phases use Playwright (no httpx).
...
# -----------------------------------------------------------------------------
# Playwright helpers (all fetching via Playwright)
# -----------------------------------------------------------------------------
...
async def playwright_fetch(
    ...
    """Fetch URL with Playwright, wait for selector, return HTML."""
    ...
    for attempt in range(config.MAX_RETRIES):
        try:
            ...
            _logger.warning(
                "Playwright fetch failed (attempt %d/%d): %s. Retrying in %.1fs",
                attempt + 1,
                config.MAX_RETRIES,
                e,
                wait,
...
def _extract_id_from_url(url: str) -> str | None:
    """Extract product ID from URL path /a/{id}/"""
    if not url:
        return None
...
def _strip_dt(text: str) -> str:
    """Strip 'DT' suffix from price strings."""
    if not text:
        return ""
    return text.replace("DT", "").strip()


def _extract_cat_id_from_url(url: str) -> str | None:
    """Extract category ID from URL for pagination."""
    if not url:
        return None
    m = re.search(r"categorie=(\d+)|/categorie[/-](\d+)|/(\d+)(?:-|$)", url)
    if m:
...
# -----------------------------------------------------------------------------
# Categories (Playwright + selectolax)
# -----------------------------------------------------------------------------
...
async def scrape_categories() -> list[dict]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_selector(
                config.CATEGORY_SELECTORS["nav_container"],
                timeout=config.PLAYWRIGHT_TIMEOUT,
                state="attached",
            )
            ...
            _logger.error("Category fetch failed: %s", e)
            return []
        finally:
            await browser.close()
    ...
    for top_idx, top_li in enumerate(top_items):
        ...
        for col in mega_panel.css("div.col-3"):
            ...
            low_name_el = low_a.css_first(cs.get("low_name", "span.u-header__sub-menu-title"))
            ...
            for sub_a in col.css(cs.get("sub_items", "ul.u-header__sub-menu-nav-group li a")):
...
# -----------------------------------------------------------------------------
# Listing Scraping (Playwright)
# -----------------------------------------------------------------------------


def parse_listing_page(html: str, category_url: str) -> list[dict]:
    tree = HTMLParser(html)
    ...
    for elem in elements:
        ...
        price_el = elem.css_first(ls.get("price"))
        ...
        old_price = _strip_dt(parse_node_text(old_price_el)) if old_price_el else ""
        ...
        if badge_stock:
            if badge_stock.css_first("i.fa.fa-check") or badge_stock.css_first("i.fa-check"):
                availability = "En stock"
            elif badge_stock.css_first("i.fa.fa-ban") or badge_stock.css_first("i.fa-ban"):
...
def get_next_page_url(html: str, current_url: str) -> str | None:
    ...
    next_icon = tree.css_first(ps.get("next_page", "i.fa-angle-right"))
    if not next_icon:
        next_icon = tree.css_first("i.fa.fa-angle-right")
    if not next_icon:
        return None
    ...
    url_pattern = ps.get("url_pattern", "?mayor=offre&mayor=shop&categorie={cat_id}&perpage=20&page={n}")
    cat_id = _extract_cat_id_from_url(current_url)
    if not cat_id:
        return None
    ...
    next_url = f"{base_url}{url_pattern.format(cat_id=cat_id, n=next_page)}"
    return next_url


async def scrape_category_listings_playwright(
    page: Page,
    category: dict,
    queue: QueueFile,
) -> list[dict]:
    ...
    try:
        while url:
            html, err = await playwright_fetch(
                page,
                url,
                "li.col-md-mc-5.col-fix.item-prod, ul.row, body",
            )
            if err or not html:
                await queue.move_to_error(key, str(err))
                return all_products

            products = parse_listing_page(html, category["url"])
...
async def scrape_all_listings(
    ...
    async with async_playwright() as p:
        ...
        async def process_cat(cat: dict) -> list[dict]:
            async with semaphore:
                return await scrape_category_listings_playwright(page, cat, queue)

        for cat in categories:
            results = await process_cat(cat)
            for p in results:
                pid = str(p.get("id", ""))
...
def parse_product_detail(html: str, url: str) -> dict:
    ...
    title = _text(sel.get("title", "h1"))

    price_el = tree.css_first(sel.get("price", "ins.font-size-36"))
    price_raw = parse_node_text(price_el)
    price = _strip_dt(price_raw) or price_raw

    schema_price = _attr(sel.get("price_schema", "meta[itemprop='price']"), "content")
    price_numeric = _strip_dt(schema_price) or price

    old_price = _strip_dt(_text(sel.get("old_price", "del")))
    discount = _text(sel.get("discount", ""))

    availability = ""
    schema_avail = tree.css_first("link[itemprop='availability']")
    if schema_avail:
        href = parse_node_attr(schema_avail, "href") or ""
        if "InStock" in href:
            availability = "En stock"
        elif "OutOfStock" in href:
            availability = "Rupture de stock"

    desc_el = tree.css_first(sel.get("description", "div#tab-description"))
    description = parse_node_text(desc_el) if desc_el else ""

    images_list: list[str] = []
    imgs_config = sel.get("images", {})
    gallery_sel = imgs_config.get("all") or (imgs_config.get("gallery", "div.fotorama") + " img")
    for img_el in tree.css(gallery_sel):
...
async def scrape_details_for_urls_playwright(
    urls: list[str],
        ...
        for url in urls:
            try:
                html, err = await playwright_fetch(
                    page,
                    url,
                    "h1, div#tab-description, div.fotorama",
                )
                if err or not html:
                    errors += 1
                    continue
                d = parse_product_detail(html, url)
                if d.get("id"):
                    details[str(d["id"])] = d
            except Exception as e:
                _logger.warning("Detail fetch failed %s: %s", url, e)
                errors += 1
        ...
        if prev_run and prev_run != run_dir:
            ...
            if prev_products_path.exists() and prev_details_path.exists():
                ...
                if not new_list and not changed_list and not removed_ids:
                    ...
                    if urls_to_scrape:
                        detail_results, det_errors = await scrape_details_for_urls_playwright(urls_to_scrape)
                ...
                details_by_id, det_errors = await scrape_details_for_urls_playwright(urls)
                stats["error_count"] += det_errors
                details_path = run_dir / config.OUTPUT_DETAILS_RAW
